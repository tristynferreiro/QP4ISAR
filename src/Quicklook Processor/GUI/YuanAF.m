...
% Implements the Yuan autofocus algorithm
    ...
    %     are available.
    %   - A threshold was introduced to ensure that only scatterers 
    %       within the object are considered. i.e. no noisy scatterers are 
    %       used. This prevents false triggering in DS selection 
    %   - NEW: Cleaning and runtime optimisation changes

    %% Variables
    num_range_bins = size(RA_HRRP,2);
    
    %% Step 1: Calculate mean and variance
    amplitudes = abs(RA_HRRP);
    amplitude_mean = mean(amplitudes,1);
    amplitude_variance = var(amplitudes,1);
    ...
    [~,candidate_scatterers_sorted] = sort(dispersion(candidate_scatterers_idx));
    ...
    product_vector = conj(ref_bins).* RA_HRRP(:,DS_idx); %RA_HRRP;
