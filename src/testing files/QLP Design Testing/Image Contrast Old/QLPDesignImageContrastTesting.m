function [contrast_original,contrast_after] = QLPDesignImageContrastTesting(CPTWL,overlap_percentage,RA_selection,AF_selection,video_name)
% Setup used for image contrast (IC) testing for QLP design considerations.
%   This function was used to IC testing of all RA and AF
%   combinations for all frames in the data. This was used when making
%   design decision for the final QLP design. 
%
%   Returns: array of contrast value of the original and focused ISAR
...
%% (A) Image Contrast testing setup
contrast_original = zeros(num_frames,1);
contrast_after = zeros(num_frames,1);
...
for frame = 1: num_frames
    ...
    contrast_original(frame) = imageContrast(ISAR_image);
    ...
    %% Plot Focused ISAR image
    contrast_after(frame) = imageContrast(ISAR_image); % calculate IC
    ...
    title(['ISAR ' num2str(frame) ' of ' num2str(num_frames)  ' - C = ' num2str(contrast_after(frame))]);
