...
% Implements the Yuan autofocus algorithm
    ...
    figure; imagesc(20*log10(abs(filtered_HRRP))); colormap('jet'); colorbar;
    xlabel('Range (m)'); ylabel('Profile Number');
    title('Filtered HRR Profiles'); colorbar;

    % Plot Candidate Scatterers: HRRP profiles 
    no_noise_HRRP = zeros(size(RA_HRRP,1), size(RA_HRRP,2));
    no_noise_HRRP(:,candidate_scatterers_idx) = RA_HRRP(:,candidate_scatterers_idx);
    figure; imagesc(20*log10(abs(no_noise_HRRP))); colormap('jet'); colorbar;
    xlabel('Range (m)'); ylabel('Profile Number');
