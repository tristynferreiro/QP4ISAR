...
% Implements the Haywood autofocus algorithm
    ...
    %   - NEW: Restructured the code for optimisation. Added additional
    %     step comments

    %% Variables
    num_range_bins = size(RA_HRRP,2);
    amplitude = abs(RA_HRRP);
    ...
    amplitude_variance = var(amplitude(:,candidate_scatterers_idx),1); % 1xM matrix
    ...
    % yline(scaling_factor*average_power_scatterer,'-g');
    ...
    % 
    % all_amplitude_var = var(amplitude,1);
    % figure; plot(var(amplitude,1), '-*'); hold on; 
